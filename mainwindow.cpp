...
{
    QString file_name = QFileDialog::getOpenFileName(this, tr("Open File"), QDir::homePath(), tr("Images (*.png *.xpm *.jpg *.jpng *.jpeg)"));
    ...
    if (!file_name.isEmpty()){
        ...
        if (suffix != "png" && suffix != "jpg" && suffix != "jpeg") {
            QMessageBox::critical(this, "Error", "Unsupported image format. Please choose PNG, JPG, or JPEG.");
            return;}
            QImage img(file_name);
        QPixmap pix = QPixmap::fromImage(img);
        qint64 w = ui->image->width();
        qint64 h = ui->image->height();
        ...
        ui->dimension->setText(QString::fromStdString("W: " + std::to_string(cols) + "  H: " + std::to_string(rows)));
        float pD = ((float)numBlackPixels/(float)(cols*rows))*100;
        ui->occu->setText(QString::fromStdString(std::to_string(pD)));

        //process image: find number of blobs
        QVector<int> results = this->findProperties(imgArray, rows, cols);
        //update the UI with the results
        ui->blobs->setText(QString::fromStdString(std::to_string(results[0])));
        ui->max->setText(QString::fromStdString(std::to_string(results[1])));
    }
    else{
        QMessageBox::information(this,"Error","Unable to Open Picture");
    }
